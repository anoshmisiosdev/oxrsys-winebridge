 ...
 *
 * This variant (derived from d3d11_draw_test.cpp, which proved a plain draw
 * call renders correctly) adds a local per-eye depth-stencil buffer, bound
 * alongside the color RTV with depth test+write enabled, matching what
 * OpenXRSamples' main.cpp does but that the plain draw test didn't exercise.
 * Isolates whether a bound depth attachment breaks DXMT's color-attachment
 * output (the leading hypothesis for the OpenXRSamples grey screen). */
#include <windows.h>
#include <d3d11.h>
...
int main(void)
{
    ...
    const char *shaderSrc =
        ...
        "VOut vs(float3 p : POSITION) { VOut o; o.pos = float4(p, 1); return o; }\n"
    ...
    D3D11_INPUT_ELEMENT_DESC ilDesc[] = { { "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 } };
    dev->CreateInputLayout(ilDesc, 1, vsBlob->GetBufferPointer(), vsBlob->GetBufferSize(), &layout);

    float verts[] = { -1, -1, 0.5f,  -1, 0, 0.5f,  0, -1, 0.5f }; /* triangle in the lower-left quadrant of clip space, mid-depth */
    ...
    for (int eye = 0; eye < 2; eye++) {
        ...
        hr = dev->CreateDepthStencilView(depthTex[eye], nullptr, &dsv[eye]);
        if (FAILED(hr)) { printf("FAIL: CreateDepthStencilView hr=%#lx\n", (unsigned long)hr); return 1; }
        printf("    eye %d: depth buffer created\n", eye);
    }
    ...
    for (int f = 0; f < 27000; f++) {   /* ~5min at 90 Hz */
        ...
        for (int eye = 0; eye < 2; eye++) {
             ...
             * clear color in the lower-left quadrant of each eye. Depth
             * buffer is bound and depth test/write are enabled, unlike
             * d3d11_draw_test.cpp. */
